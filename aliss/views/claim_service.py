...
from aliss.models import Claim, Service
...
class ClaimDetailView(StaffuserRequiredMixin, UpdateView):
    ...
    def send_user_email(self, claim):
        if claim.status == Claim.CONFIRMED:
            ...
            message = 'Congratulations, your claim of ownership for {service} on ALISS has been confirmed.'.format(service=claim.service)
        elif claim.status == Claim.REVOKED:
            message = 'We are sorry, but your claim of ownership for {service} on ALISS has been revoked.'.format(
                service=claim.service
            )
        elif claim.status == Claim.DENIED:
            message = 'We are sorry, but your claim of ownership for {service} on ALISS has been denied.'.format(
                service=claim.service
            )
        else:
            return False
        ...
        send_mail(
            'Your ownership claim of {service} on ALISS'.format(
                service=claim.service
            ),
            message,
    ...
    def form_valid(self, form):
        ...
        if self.object.status == Claim.CONFIRMED:
            self.object.service.claimed_by = self.object.user
            self.object.service.save()
        
        elif self.object.status == Claim.REVOKED:
            self.object.service.claimed_by = None
            self.object.service.save()
...
class ClaimCreateView(LoginRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        service = get_object_or_404(service, pk=self.kwargs.get('pk'))
        form = ClaimForm(initial={ 'phone': self.request.user.phone_number })
        return self.render_to_response(
            self.get_context_data(service=service, form=form)
        )
...
class ClaimDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    success_url = reverse_lazy('account_my_services')
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if self.object.quit_representation():
            messages.success(self.request,
                'You have stopped representing this service.'
            )
        else:
